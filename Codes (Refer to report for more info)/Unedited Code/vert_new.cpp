...
void loop() {
    ...
    if (trueDepth < 0.9 * Setpoint) {
      ...
      // Invert the Output for positive buoyancy correction 
      // thrusterPower = initThrusterValue - (Output * 100); 
      
    //   int pwmValue = constrain(thrusterPower, 1100, 1900);  // Ensure PWM value is between 1000 and 2000 
  
    //   for (int i = 0; i < 4; i++) { 
    //     setThrusterPWM(thrusterPins[i], pwmValue); // Adjust thruster PWM value 
    //   } 
  
    //   Serial.print("Thruster Power: "); 
    //   Serial.print(thrusterPower); 
...
void setThrusterPWM(int thrusterPin, int value) {
   value = constrain(value, 1100, 1900);
   for (int i=0;i<4;i++) {
